/**
 * Signal Backtester Page
 *
 * Shows historical performance of SIBT traffic light signals.
 * "If you only traded on TRADE days, here's what happened."
 */
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {(["3M", "6M", "1Y"] as Period[]).map((p) => {
            ...
            return (
              ...
              >
                {p}{disabled && !canAccessBasic ? " (Pro)" : disabled ? " (Pro)" : ""}
              </button>
            );
          })}
          <button
      ...
      {result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
            <StatCard
              label="SIBT Return"
              value={`${result.sibtReturn >= 0 ? "+" : ""}${result.sibtReturn}%`}
              color={result.sibtReturn >= 0 ? "#05AD98" : "#E85D6C"}
              detail={`Following TRADE signals only (${period})`}
            />
            <StatCard
              label="Buy & Hold SPY"
              value={`${result.buyHoldReturn >= 0 ? "+" : ""}${result.buyHoldReturn}%`}
              color={result.buyHoldReturn >= 0 ? "#05AD98" : "#E85D6C"}
              detail="Holding SPY every day"
            />
            <StatCard
              label="Outperformance"
              value={`${result.outperformance >= 0 ? "+" : ""}${result.outperformance}%`}
              color={result.outperformance >= 0 ? "#05AD98" : "#E85D6C"}
              detail="SIBT minus buy-and-hold"
            />
            <StatCard
              label="Win Rate"
              value={`${result.winRate}%`}
              color={result.winRate >= 55 ? "#05AD98" : result.winRate >= 45 ? "#F5A623" : "#E85D6C"}
              detail={`Of ${result.tradeDays} TRADE days`}
            />
            <StatCard
              label="Max Drawdown"
              value={`${result.maxDrawdown}%`}
              color={result.maxDrawdown > -5 ? "#05AD98" : result.maxDrawdown > -10 ? "#F5A623" : "#E85D6C"}
              detail="Worst peak-to-trough"
            />
            <StatCard
              label="Avg Win / Loss"
              value={`+${result.avgWinDay}% / ${result.avgLoseDay}%`}
              color="var(--text-primary)"
              detail="Average day returns"
            />
          </div>
...
function EquityCurve({ data }: { data: { date: string; sibt: number; buyHold: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 250 }}>
      ...
      {data.length > 0 && (
        <>
          <text x={W - PAD} y={toY(data[data.length - 1].sibt) - 6} fontFamily="var(--font-mono)" fontSize={9} fill="#05AD98" textAnchor="end">
            ${data[data.length - 1].sibt.toFixed(0)}
          </text>
          <text x={W - PAD} y={toY(data[data.length - 1].buyHold) + 12} fontFamily="var(--font-mono)" fontSize={9} fill="#94a3b8" textAnchor="end">
            ${data[data.length - 1].buyHold.toFixed(0)}
          </text>
        </>
      )}
    </svg>
