/**
 * Earnings calendar content — extracted from EarningsPage.
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        <FilterChip label="ALL" active={!sectorFilter} onClick={() => setSectorFilter(null)} color="var(--signal-core)" />
        {sectors.map((s) => (
          <FilterChip
            key={s}
            label={s.toUpperCase()}
      ...
      {loading && earnings.length === 0 ? (
        <Panel title="Loading...">
          <div style={{ padding: 16, fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--text-muted)" }}>Fetching earnings calendar...</div>
        </Panel>
      ) : (
                ...
                {entries.map((e, i) => {
                  ...
                  return (
                    <tr key={`${e.symbol}-${i}`} style={{ borderBottom: "1px solid var(--border-dim)", height: 32 }}>
                      ...
                      <td style={{ padding: "0 8px", fontWeight: 700, color: "var(--text-primary)" }}>{e.symbol}</td>
                      <td style={{ padding: "0 8px" }}>
                        <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 999, fontSize: 11, color: SECTOR_COLORS[e.sector ?? "Other"], border: `1px solid ${SECTOR_COLORS[e.sector ?? "Other"]}` }}>
                          {e.sector}
                        </span>
                      </td>
                      <td style={{ padding: "0 8px" }}>
                        <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 999, fontSize: 11, fontWeight: 600, color: hourCfg.color, border: `1px solid ${hourCfg.color}` }}>
                          {hourCfg.label}
                        </span>
                      </td>
                      <td style={{ padding: "0 8px", textAlign: "right", color: "var(--text-secondary)" }}>{formatEps(e.epsEstimate)}</td>
                      <td style={{ padding: "0 8px", textAlign: "right", color: "var(--text-secondary)" }}>{formatRevenue(e.revenueEstimate)}</td>
                      ...
                      <td style={{ padding: "0 8px" }}>
                        <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 999, fontSize: 11, fontWeight: 600, color: expectation.color, border: `1px solid ${expectation.color}` }}>
                          {expectation.label}
                        </span>
                      </td>
                      <td style={{ padding: "0 8px" }}>
                        {e.analyst ? (
                          <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 999, fontSize: 11, fontWeight: 600, color: e.analyst.buyPct >= 70 ? "var(--positive)" : e.analyst.buyPct >= 40 ? "var(--warning)" : "var(--negative)", border: `1px solid ${e.analyst.buyPct >= 70 ? "var(--positive)" : e.analyst.buyPct >= 40 ? "var(--warning)" : "var(--negative)"}` }}>
                          ...
                          <div style={{ display: "flex", gap: 2 }}>
                            {e.epsSurprises.slice(0, 4).map((s, si) => {
                              ...
                              return (
                                <span
                                  key={si}
                                  title={`Q${s.quarter} ${s.year}: ${isBeat ? "BEAT" : isMiss ? "MISS" : "IN-LINE"} ${isBeat ? "+" : ""}${s.surprisePercent.toFixed(1)}%`}
                                  style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 14, height: 14, borderRadius: 2, padding: "0 2px", fontSize: 9, fontWeight: 700, fontFamily: "var(--font-mono)", background: isBeat ? "var(--positive)" : isMiss ? "var(--negative)" : "var(--text-muted)", color: "#000", opacity: 0.9, cursor: "default" }}
                                >
                                  {isBeat ? "B" : isMiss ? "M" : "="}
                                </span>
                              );
                      ...
                      <td style={{ padding: "0 8px" }}>
                        <div style={{ display: "flex", gap: 4 }}>
                          <button
                            ...
                            style={{ padding: "1px 6px", borderRadius: 999, fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 600, color: "var(--info)", background: "transparent", border: "1px solid var(--info)", cursor: "pointer", opacity: 0.8, whiteSpace: "nowrap" }}
                          >
                            INTEL
                          </button>
                          {view === "past" && e.epsActual != null && (
                            <button
                              ...
                              style={{ padding: "1px 6px", borderRadius: 999, fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 600, color: "var(--signal-core)", background: "transparent", border: "1px solid var(--signal-core)", cursor: "pointer", opacity: 0.8, whiteSpace: "nowrap" }}
                            >
                              AI
                            </button>
                          )}
      ...
      {filtered.length === 0 && !loading && (
        <Panel title="No Earnings">
          <div style={{ padding: 16, fontFamily: "var(--font-mono)", fontSize: 13, color: "var(--text-muted)" }}>No major earnings found for the selected filters.</div>
        </Panel>
      )}
      ...
      <div style={{ padding: "8px 12px", fontFamily: "var(--font-sans)", fontSize: 12, color: "var(--text-muted)", fontStyle: "italic", borderTop: "1px solid var(--border-dim)" }}>
        Earnings data from Finnhub. EPS and revenue estimates are consensus analyst forecasts. Results and expectations are for informational purposes only and do not constitute investment advice.
      </div>

      {summaryTarget && (
        <EarningsIntelPanel symbol={summaryTarget.symbol} quarter={summaryTarget.quarter} year={summaryTarget.year} earningsDate={summaryTarget.earningsDate} currentEpsEstimate={summaryTarget.epsEstimate} currentRevEstimate={summaryTarget.revenueEstimate} onClose={() => setSummaryTarget(null)} />
      )}

      {aiSummaryTarget && (
        <EarningsSummaryPanel symbol={aiSummaryTarget.symbol} quarter={aiSummaryTarget.quarter} year={aiSummaryTarget.year} earningsDate={aiSummaryTarget.earningsDate} onClose={() => setAiSummaryTarget(null)} />
      )}
    </div>
  );
}
...
function FilterChip({ label, count, active, onClick, color }: { label: string; count?: number; active: boolean; onClick: () => void; color: string }) {
  return (
    <button
      onClick={onClick}
      style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 10px", borderRadius: 999, fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 600, color: active ? "#000" : color, background: active ? color : "transparent", border: `1px solid ${color}`, cursor: "pointer", opacity: active ? 1 : 0.7 }}
    >
      {label}
