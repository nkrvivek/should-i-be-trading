      ...
      {result && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
            <StatCard label="SIBT Return" value={`${result.sibtReturn >= 0 ? "+" : ""}${result.sibtReturn}%`} color={result.sibtReturn >= 0 ? "#00c805" : "#E85D6C"} detail={`Following TRADE signals only (${period})`} />
            <StatCard label="Buy & Hold SPY" value={`${result.buyHoldReturn >= 0 ? "+" : ""}${result.buyHoldReturn}%`} color={result.buyHoldReturn >= 0 ? "#00c805" : "#E85D6C"} detail="Holding SPY every day" />
            <StatCard label="Outperformance" value={`${result.outperformance >= 0 ? "+" : ""}${result.outperformance}%`} color={result.outperformance >= 0 ? "#00c805" : "#E85D6C"} detail="SIBT minus buy-and-hold" />
            <StatCard label="Win Rate" value={`${result.winRate}%`} color={result.winRate >= 55 ? "#00c805" : result.winRate >= 45 ? "#F5A623" : "#E85D6C"} detail={`Of ${result.tradeDays} TRADE days`} />
            <StatCard label="Max Drawdown" value={`${result.maxDrawdown}%`} color={result.maxDrawdown > -5 ? "#00c805" : result.maxDrawdown > -10 ? "#F5A623" : "#E85D6C"} detail="Worst peak-to-trough" />
          ...
          <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border-dim)", borderRadius: 4, padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 2, height: 24, borderRadius: 4, overflow: "hidden" }}>
              <div style={{ flex: result.tradeDays, background: "#00c805", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {result.equityCurve.length > 0 && (
            <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border-dim)", borderRadius: 4, padding: 16 }}>
              ...
              <div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 8 }}>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "#00c805" }}>&#9632; SIBT Strategy</span>
              ...
              <tbody>
                {result.dailyReturns.slice(-20).reverse().map((d) => (
                  <tr key={d.date} style={{ borderBottom: "1px solid var(--border-dim)" }}>
                    ...
                    <td style={{ padding: "4px 8px", textAlign: "right", color: d.spyChange >= 0 ? "#00c805" : "#E85D6C" }}>
...
function EquityCurve({ data }: { data: { date: string; sibt: number; buyHold: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxHeight: 250 }}>
      ...
      <path d={sibtPath} fill="none" stroke="#00c805" strokeWidth={2} />
      {data.length > 0 && (
        <>
          <text x={W - PAD} y={toY(data[data.length - 1].sibt) - 6} fontFamily="var(--font-mono)" fontSize={9} fill="#00c805" textAnchor="end">${data[data.length - 1].sibt.toFixed(0)}</text>
